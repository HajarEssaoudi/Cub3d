...
/*   Updated: 2025/10/31 00:53:27 by hqannouc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_mini_fov(t_game *game)
{
	...
	while (ray < game->win_width)
	{
		init_ray(game, r_angle, &ray_data);
		dist = count_dist(game, r_angle, &ray_data);
		x = (game->player.x + cos(r_angle) * dist) * 15;
		y = (game->player.y + sin(r_angle) * dist) * 15;
		my_mlx_pixel_put(game->img_data, (int)x, (int)y, 0xFFFF00);
		r_angle += deg_to_rad(FOV) / game->win_width;
		ray++;
	}
