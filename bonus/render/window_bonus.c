...
/*   Updated: 2025/12/08 22:05:04 by hqannouc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	to_be_collided(t_game *game, double y, double x)
{
	if (y > 1 && y < (game->scene.map_height - 1)
		&& x > 1 && x < (game->scene.map_width - 1)
		&& game->scene.map_pad[(int)(y)][(int)(x)] != '1'
		&& game->scene.map_pad[(int)(y)][(int)(x)] != '\0'
		&& game->scene.map_pad[(int)(y)][(int)(x)] != '_'
		&& game->scene.map_pad[(int)(y)][(int)(x)] != 'D')
		return (1);
	else
		return (0);
}
