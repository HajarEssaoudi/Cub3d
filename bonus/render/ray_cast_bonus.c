...
/*   Updated: 2025/12/08 18:54:31 by hqannouc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	dda_loop(t_game *game, t_ray *ray, double *side_dx, double *side_dy)
{
	while (1)
	{
		if (*side_dx <= *side_dy)
		{
			...
			if (game->scene.map_pad[ray->map_y][ray->map_x] != '0'
				&& game->scene.map_pad[ray->map_y][ray->map_x] != 'd'
				&& !is_player(game->scene.map_pad[ray->map_y][ray->map_x]))
				return (game->scene.map_pad[ray->map_y][ray->map_x]);
			else
				*side_dx += ray->delta_x;
		}
		else
